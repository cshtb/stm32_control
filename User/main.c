			  ...
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
						{
									...
									sprintf ( cStr_1, "Temperature  :  %.2f  ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									LCD_DisplayStringLine(LINE(10),(uint8_t* )cStr);	
						#endif
							
						#ifdef USE_USART_DISPLAY
									sprintf ( cStr, "Pitch :  %.4f  ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									printf("%s  ",(uint8_t* )cStr);			
						...
						{																									
						#ifdef USE_USART_DISPLAY
									sprintf ( cStr_3, "gyro_x :  %.4f  ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_gyro读出的数据是Q16格式，所以左移16位.		
									printf("%s  ", (uint8_t* )cStr_3); //输出角速度
									
									sprintf ( cStr_3, "gyro_y  :  %.4f  ", data[1]*1.0/(1<<16) );	//inv_get_sensor_type_gyro读出的数据是Q16格式，所以左移16位.	
									printf("%s  ", (uint8_t* )cStr_3); //输出角速度
									
									sprintf ( cStr_3, "gyro_z   :  %.4f  ", data[2]*1.0/(1<<16) );	//inv_get_sensor_type_gyro读出的数据是Q16格式，所以左移16位.
